...
# class HttpProxyMiddleware(object):
    ...
    #     request.meta['proxy'] = "http://111.187.5.7:23451"
#
# class HttpProxyMiddleware(object):
...
#         self.extend_proxy_threshold = 20
#         # 初始化代理列表
...
#         从网上抓取新的代理添加到代理列表中
#         """
#         print("extending proxyes using fetch_free_proxyes.py")
#         new_proxyes = fetch_free_proxyes.fetch_all()
#         print("new proxyes: %s" % new_proxyes)
#         self.last_fetch_proxy_time = datetime.now()
#
...
#             print("valid proxy < threshold: %d/%d" % (self.len_valid_proxy(), self.extend_proxy_threshold))
#             self.fetch_new_proxyes()
#
#         print("正使用新代理: %s" % self.proxyes[self.proxy_index]["proxy"])
#
...
#             print("invalidate %s" % self.proxyes[index])
...
#             print("After %d minutes later, recover from using proxy" % self.recover_interval)
#             self.last_no_proxy_time = datetime.now()
#             self.proxy_index = 0
...
#             print("change proxy request get by spider: %s" % request)
...
#         # else:
#             # print("None %s %s" % (response.status, request.url))
#
...
#             print("状态码不在spider.website_possible_httpstatus_list中: {url}, {status}".format(
#                 url=response.url,
...
#         处理由于使用代理导致的连接异常
#         """
#         print("%s exception: %s" % (self.proxyes[request.meta["proxy_index"]]["proxy"], exception))
...
#                 # WARNING 直连时超时的话换个代理还是重试? 这是策略问题
